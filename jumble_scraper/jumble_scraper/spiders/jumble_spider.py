...
class JumbleSolverSpider(scrapy.Spider):
    name: str = "jumble_solver_spider"

    def __init__(
        self,
        end_date: str | date = date.today(),
        raw_url: str = "https://jumblesolver.com",
        answers_endpoint: str = "/daily-jumble-answers",
        date_offset: int = 10,
    ...
    def start_requests(self):
        for value_date in self.value_dates:
            url = f"{self.raw_url}{self.answer_endpoint}/{value_date.strftime(self.date_format)}"
            yield scrapy.Request(url, callback=self.parse_jumbles)

    def parse_jumble_solution(self, response):
        item: JumbleSolverItem = response.meta["item"]
    ...
    def parse_jumbles(self, response):
        ...
        for button in buttons:
            ...
            item = JumbleSolverItem(url_from=response.url, jumbled=jumbled)
